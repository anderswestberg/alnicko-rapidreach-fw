/**
 * @file cavli-c16qs.c
 * @brief Cavli C16QS modem driver based on Zephyr's modem_cellular.c
 *
 * @author Eugene K.
...
LOG_MODULE_REGISTER(modem_cavli_c16qs, CONFIG_MODEM_LOG_LEVEL);
...
#define CMUX_SABM_ESTABLISHING "\xF9\x03\x3F\x01\x1C\xF9"
#define CMUX_SABM_DLCI1 "\xF9\x07\x3F\x01\xDE\xF9"
#define CMUX_SABM_DLCI2 "\xF9\x0B\x3F\x01\x59\xF9"
#define CMUX_SABM_DLCI3 "\xF9\x0F\x3F\x01\x9B\xF9"
...
MODEM_CHAT_MATCH_DEFINE(cgmm_match, "+CGMM:", "", modem_cellular_chat_on_cgmm);
...
MODEM_CHAT_MATCH_DEFINE(cgmi_match __maybe_unused, "+CGMI:", "", modem_cellular_chat_on_cgmi);
MODEM_CHAT_MATCH_DEFINE(cgmr_match __maybe_unused, "+CGMR:", "", modem_cellular_chat_on_cgmr);
...
/*
 * The modem uses two custom scripts to initialize the modem and dial out.
 *
...
#if DT_HAS_COMPAT_STATUS_OKAY(cavli_c16qs) 
MODEM_CHAT_SCRIPT_CMDS_DEFINE(cavli_c16qs_init_chat_script_cmds,
			      MODEM_CHAT_SCRIPT_CMD_RESP("ATE0", ok_match),
#ifdef CONFIG_MODEM_CAVLI_C16QS_ENABLE_SIMSWAP
			      MODEM_CHAT_SCRIPT_CMD_RESP_NONE("AT^SIMSWAP=1", 2000),
#endif
			      ...
			      MODEM_CHAT_SCRIPT_CMD_RESP("", ok_match),
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT+CMUX=1", ok_match),
				
			      MODEM_CHAT_SCRIPT_CMD_RESP_NONE(CMUX_SABM_ESTABLISHING, 300),
				  MODEM_CHAT_SCRIPT_CMD_RESP_NONE(CMUX_SABM_DLCI1, 300),
			
				);


MODEM_CHAT_SCRIPT_DEFINE(cavli_c16qs_init_chat_script, cavli_c16qs_init_chat_script_cmds,
			 abort_matches, modem_cellular_chat_callback_handler, 10);

MODEM_CHAT_SCRIPT_CMDS_DEFINE(cavli_c16qs_dial_chat_script_cmds,
				  ...
				  MODEM_CHAT_SCRIPT_CMD_RESP_NONE("AT+CGACT?", 300),
				  MODEM_CHAT_SCRIPT_CMD_RESP_NONE("AT$QCNETDEVCTL?", 300),

			      MODEM_CHAT_SCRIPT_CMD_RESP_NONE("ATD*99***1#", 0),
				
				);

MODEM_CHAT_SCRIPT_DEFINE(cavli_c16qs_dial_chat_script, cavli_c16qs_dial_chat_script_cmds,
			 dial_abort_matches, modem_cellular_chat_callback_handler, 10);

MODEM_CHAT_SCRIPT_CMDS_DEFINE(cavli_c16qs_periodic_chat_script_cmds,
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT+CREG?", ok_match),
			      MODEM_CHAT_SCRIPT_CMD_RESP("AT+CEREG?", ok_match));

MODEM_CHAT_SCRIPT_DEFINE(cavli_c16qs_periodic_chat_script,
			 cavli_c16qs_periodic_chat_script_cmds, abort_matches,
...
#define MODEM_CELLULAR_DEVICE_CAVLI_C16QS(inst)                                                   \
	...
	MODEM_CELLULAR_DEFINE_INSTANCE(inst, 1500, 100, 10000, 5000, false,                        \
				       NULL,                                                       \
				       &cavli_c16qs_init_chat_script,                             \
				       &cavli_c16qs_dial_chat_script,                             \
				       &cavli_c16qs_periodic_chat_script, NULL)


#define DT_DRV_COMPAT cavli_c16qs
DT_INST_FOREACH_STATUS_OKAY(MODEM_CELLULAR_DEVICE_CAVLI_C16QS)
#undef DT_DRV_COMPAT
